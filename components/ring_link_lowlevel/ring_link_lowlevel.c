...
esp_err_t ring_link_lowlevel_transmit_payload(ring_link_payload_t *p)
{
    ...
    if( xSemaphoreTake( s_tx_semaphore_handle, ( TickType_t ) 1000 ) == pdTRUE )
    {
        ...
        ESP_LOGW(TAG, "Mensaje ring_link_lowlevel_transmit_payload: en %lld μs", duration);
        return rc;
    }
    printf("Falla la transmision");
